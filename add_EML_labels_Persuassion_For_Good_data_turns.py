...
# ===== PROMPT TEMPLATE FOR SINGLE TURN CLASSIFICATION =====
EML = """
...
examples_turn = f"""
    ...
    Output: 1
    Explanation: The text contains emotional manipulative language since the speaker is asking the receiver to do the homework by using authority and minimizing his/her emotions (i.e. not caring if he/she feels tired).
    ...
    Output: 1
    Explanation: Emotional appeals (shame) is used to influence the receiver (to come there or, probably, to stop some kind of behavior).
...
def generate_prompt():
    return f"""
        ...
        * The text is used to influence someone in doing or thinking something, not relying on reasoned argument but on the exploitation of emotional appeals to achieve compliance.
        * The text includes at least one of the following language techniques of manipulation:
        ...
        TASK:
        Classify whether the text contains emotionally manipulative language (1) or not (0). Return a single digit.
...
def predict_single(text, tokenizer, model):
    ...
    # Extract first digit 0 or 1, default to 0 if none found
